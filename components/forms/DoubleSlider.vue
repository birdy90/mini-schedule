<template>
  <div>
    ...
    <div class="bg-gray-100 w-full h-2 rounded mb-2 relative">
      <div
        class="bg-main-300 h-full absolute top-0"
        :style="currentRangeStyles"
      >
        <div
          tabindex="0"
          :class="[handleClasses, '-left-1']"
          @keydown.left="moveLeft(0)"
          @keydown.right="moveRight(0)"
        />
        <div
          tabindex="0"
          :class="[handleClasses, '-right-1']"
          @keydown.left="moveLeft(1)"
          @keydown.right="moveRight(1)"
        />
      </div>
    </div>
  </div>
...
function moveLeft(index: number) {
  ...
  const canMove =
    newValue.filter((t) => t === newValue[index]).length &&
    newValue[index] >= range.value[0];
  if (!canMove) return;
